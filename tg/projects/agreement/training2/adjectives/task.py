...
from tg.projects.agreement.training2.adjectives.common import MulticlassPredictionInterpreter
from tg.projects.agreement.training2.adjectives.metrics import AlternativeTaskMulticlassMetrics
...
def get_multilabel_extractor():
    label_extractor = (bt.PlainExtractor
                       ...
                       .apply(take_columns=['label'],
                              transformer=dft.DataFrameTransformerFactory.default_factory(max_values_per_category=1000))
                       )
    return label_extractor


class AdjectivesTrainingTask(btf.TorchTrainingTask):
    def __init__(self, *, assembly_point: btc.ContextualAssemblyPoint, network_factory: NetworkFactory,
                 label_count: int):
        super(AdjectivesTrainingTask, self).__init__()
        self.metric_pool = bt.MetricPool().add(AlternativeTaskMulticlassMetrics(label_count=label_count))
        self.assembly_point = assembly_point
...
def create_empty_training_task(
    ...
    task = AdjectivesTrainingTask(assembly_point=ap, network_factory=network_factory, label_count=label_count)
...
def train_adjectives_model(idb: bt.IndexedDataBundle, save_path: Path, **task_kwargs) -> None:
