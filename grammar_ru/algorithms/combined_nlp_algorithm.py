...
class CombinedNlpAlgorithm(NlpAlgorithm):
    def __init__(self, algorithms: List[NlpAlgorithm]):
        super(CombinedNlpAlgorithm, self).__init__('status', 'suggestion','hint')
    ...
    @staticmethod
    def merge_algorithms(df, algorithms, status_column='status', suggestion_column='suggestion', hint_column='hint', algorithm_column='algorithm'):
        statuses = []
        suggestions = []
        hints = []
        for alg in algorithms:
            ...
            hints.append(alg.get_hint_column())
        ...
        for i in range(len(statuses)):
            ...
            if hints[i] is not None:
                df.loc[current, hint_column] = df.loc[current, hints[i]]
