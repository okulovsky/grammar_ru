...
from grammar_ru.algorithms import NlpAlgorithm, CombinedNlpAlgorithm


class FakeNlpAlgorithm(NlpAlgorithm):
    def __init__(self, n):
        super(FakeNlpAlgorithm, self).__init__(f'status_{n}', f'suggestion_{n}', f'hint_{n}')
        self.n = n

    def _run_inner(self, df):
        df[self.get_status_column()] = df.index % self.n != 0
        df[self.get_suggest_column()] = str(self.n)
        df[self.get_hint_column()] = str(self.n)
